        ...
        try:
            ...
            for sheet in xls.sheet_names:
                ...
                if 1 == 1:
                    ...
                    df_raw = df_raw[df_raw['Cód EAN'].notna()]
                    df2 = pd.DataFrame()
                    df2['EAN'] = normalizar_ean(df_raw['Cód EAN'].astype(str).str.zfill(13))
                    df2['MÊS'] = '02/01/2025'
                    df2['CANAL'] = 'REDE'
                    df2['CLIENTE'] = 'DIMED'
                    df2['GC'] = 'JULIO OLIVEIRA'
                    df2['UF'] = 'RS'
                    df2['FAMILIA'] = df_raw['Apresentação'].astype(str).str.strip().str.split().str[0]
                    df2['SKU_SERVIER'] = df_raw['Apresentação']
                    df2['SELL OUT MÊS'] = df_raw['VENDAS']
                    df2['ESTOQUE DISPONIVEL LOJA/CD'] = df_raw['ESTQ_VAREJO']
                    df2['PENDENCIA TRANSITO/TRANSFERENCIA'] = df_raw['PENDENCIA_EDS'] + df_raw['PENDENCIA_PRN']
                    df2['PENDENCIA ENTREGA'] = ''
                    df2['ESTOQUE CD REDE'] = df_raw['ESTQ_EDS'] + df_raw['ESTQ_PRN']
